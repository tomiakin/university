...
for k=1:length(Mmin_vector)
    ...
    for j=1:length(SUB_CATALOG)
        ...
        figure(j)
        title(['Zone ',num2str(j)],'Interpreter', 'latex')
        hold on
        loglog(T,cumsum(n),'linewidth',2)
        xlim([0.9*min(T) 1*max(T)])
        grid on
        set(gca, 'TickLabelInterpreter', 'latex');
        ax = gca;
        axis square; xlabel('Time (Years)','Interpreter', 'latex'); ylabel('Cumulative number', 'Interpreter', 'latex');
        set(gca,'FontSize',20)
