...
for ti = 1:length(Tnn)
    ...
    for i=1:length(a)
        ...
        if strcmp(FaultType(i),'strike-slip')==1
            FMech = [0 0];
            faultType=3;
        elseif strcmp(FaultType(i),'normal')==1
            FMech = [1 0];
            faultType=1;
        elseif strcmp(FaultType(i),'reverse')==1
            FMech = [0 1];
            faultType=2;
        end
        %% Soil Term
        ...
        for ir=1:length(Ri)
            for jm=1:length(Mi)
                ...
                [gmp_central_estimate2,~,~,~,sigma_total]=GMPE_Bindi_DS_EC8(1,Tn,Mi(jm),Ri(ir),Vs30,faultType,table1);
                
                if counter==1
                    Pgmpe1=(1-normcdf(log10(imx),log10(gmp_central_estimate),coef1(Tnn==Tn,14))).*PRi.*PMi(jm);
                    Pgmpe2=(1-normcdf(log10(imx),log10(gmp_central_estimate2),sigma_total)).*PRi.*PMi(jm);
                else
                    Pgmpe1=Pgmpe1+(1-normcdf(log10(imx),log10(gmp_central_estimate),coef1(Tnn==Tn,14))).*PRi.*PMi(jm);
                    Pgmpe2=Pgmpe2+(1-normcdf(log10(imx),log10(gmp_central_estimate2),sigma_total)).*PRi.*PMi(jm);
                end
                
                Dgmpe1(ir,jm,:)=normpdf(log(imx)./log(10),log(gmp_central_estimate)./log(10),coef1(Tnn==Tn,14))./imx.*PRi.*PMi(jm);
                Dgmpe2(ir,jm,:)=normpdf(log(imx)./log(10),log(gmp_central_estimate2)./log(10),sigma_total)./imx.*PRi.*PMi(jm);
            end
        end
        
        
        Pgmpe = 0.5 * Pgmpe1 + 0.5 * Pgmpe2;
        Dgmpe = 0.5 * Dgmpe1 + 0.5 * Dgmpe2;
        
        Lambda_min(i) = round(10^(A-B*Mmin)*100)/100;
